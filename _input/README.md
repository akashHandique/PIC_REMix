...
> commodities `Dummy_EL` and `eKerosene` directly, *not* the `Marine_e` /
> `Aviation_e` converters their naming suggests. Per the known structural
> gap documented in `Process/README.md` §7, `Marine_e` and `Aviation_e` are
> defined in the model but never given a `converter_coefficient` — so
> these two demand columns bypass them by design (or by leftover
> mis-wiring, depending on intent) rather than routing through them.
