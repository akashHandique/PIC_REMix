...
# Define tech groups
no_feedin_techs = ["BG_N"]  # availability fixed
feedin_techs = [
    "PV_B", "WindOnshore_B", "PV_N", "WindOnshore_N", "Wave_N", "WindOffshore_N"
]  # placeholder availability

# Create the base DataFrame
converter_techParam = pd.DataFrame(
    index=pd.MultiIndex.from_product([
        no_feedin_techs + feedin_techs,
        m.set.yearssel
    ])
)

# Assign lifeTime = 25 for all
for tech in no_feedin_techs + feedin_techs:
    converter_techParam.loc[idx[tech], "lifeTime"] = 25

# Assign activityUpperLimit
converter_techParam.loc[idx[no_feedin_techs], "activityUpperLimit"] = 1
converter_techParam.loc[idx[feedin_techs], "activityUpperLimit"] = 0
...
converter_capacityParam.loc[idx["R1_data", :, "BG_N"], "unitsUpperLimit"] = 1  # GW_el # 0.167
converter_capacityParam.loc[idx["R1_data", :, "BG_N"], "unitsLowerLimit"] = 0
converter_capacityParam.loc[idx["R2_data", :, "BG_N"], "unitsUpperLimit"] = 1  # GW_el # 0.167
converter_capacityParam.loc[idx["R2_data", :, "BG_N"], "unitsLowerLimit"] = 0
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "PV_B"], "unitsUpperLimit"] = 0.0071  # GW_el # 0.167
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "PV_B"], "unitsLowerLimit"] = 0.0070   # GW_el # 0.0165
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "WindOnshore_B"], "unitsUpperLimit"] = 0.00151  # GW_el
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "WindOnshore_B"], "unitsLowerLimit"] = 0.00150  # GW_el
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "PV_N"], "unitsUpperLimit"] = (
    1  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "PV_N"], "unitsLowerLimit"] = (
   0  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "WindOnshore_N"], "unitsUpperLimit"] = (
  1  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "WindOnshore_N"], "unitsLowerLimit"] = (
    0  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "Wave_N"], "unitsUpperLimit"] = (
  1  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "Wave_N"], "unitsLowerLimit"] = (
    0  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "WindOffshore_N"], "unitsUpperLimit"] = (
  1  # GW_el
)
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "WindOffshore_N"], "unitsLowerLimit"] = (
    0  # GW_el
)
...
for data_node in ["R1_data", "R2_data"]:
...
# %%
# "sourcesink_profile"
demand_R4_R2_CH = profiles[["demand_R1", "demand_R2"]]
...
demand_R4_R2_CH = demand_R4_R2_CH.rename(
    index={"demand_R1": "R1_data", "demand_R2": "R2_data"}
)
...
sourcesink_config.loc[idx[["R1_data","R2_data"], :, :, :], "usesFixedProfile"] = 1
...
sourcesink_annualSum.loc[idx["R2_data", :, :, :], "upper"] = 511
sourcesink_annualSum.loc[idx["R2_data", :, :, :], "lower"] = 0
sourcesink_annualSum = sourcesink_annualSum.dropna()
...
sourcesink_config.loc[idx[["R1_data","R2_data"], :, :, :], "usesUpperSum"] = 1
sourcesink_config.loc[idx[["R1_data","R2_data"], :, :, :], "usesLowerProfile"] = 1
sourcesink_config = sourcesink_config.dropna()
...
accounting_sourcesinkFlow = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["FuelCost"], ["R1_data"], m.set.yearssel, ["FuelImport"], ["Biomass"]]
    )
)
...
accounting_sourcesinkFlow = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["FuelCost"], ["R2_data"], m.set.yearssel, ["FuelImport"], ["Biomass"]]
    )
)
accounting_sourcesinkFlow["perFlow"] = 0.032  # Mio EUR per GWh_ch CH4
...
sourcesink_annualSum.loc[idx[["R1_data","R2_data"],  :, :, :], "lower"] = -np.inf
sourcesink_annualSum = sourcesink_annualSum.dropna()
...
sourcesink_config.loc[idx[["R1_data","R2_data"], :, :, :], "usesLowerSum"] = 1
sourcesink_config.loc[idx[["R1_data","R2_data"], :, :, :], "usesUpperProfile"] = 1
sourcesink_config = sourcesink_config.dropna()
...
converter_capacityParam.loc[idx[["R1_data","R2_data"], :, "Battery"], "unitsUpperLimit"] = (
...
storage_reservoirParam.loc[idx[["R1_data","R2_data"], :, "Battery"], "unitsUpperLimit"] = (
    1000  # units
)
