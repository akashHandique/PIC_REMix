...
converter_techParam = pd.DataFrame(
    index=pd.MultiIndex.from_product([["CCGT", "BG", "PV", "WindOnshore", "Wave","WindOffshore"], m.set.yearssel])
)
...
converter_techParam.loc[idx["BG"], "lifeTime"] = 30  # years
converter_techParam.loc[idx["BG"], "activityUpperLimit"] = (
    1  # availability of technology
)
...
converter_techParam.loc[idx["Wave"], "lifeTime"] = 25
converter_techParam.loc[idx["Wave"], "activityUpperLimit"] = (
    0  # this value will be replaced later on with the normalized feed-in profile
)
converter_techParam.loc[idx["WindOffshore"], "lifeTime"] = 25
converter_techParam.loc[idx["WindOffshore"], "activityUpperLimit"] = (
    0  # this value will be replaced later on with the normalized feed-in profile
)
...
converter_capacityParam = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [m.set.nodesdata, m.set.yearssel, ["CCGT", "BG", "PV", "WindOnshore", "Wave", "WindOffshore"]]
    )
)
converter_capacityParam.loc[idx["R1_data", :, "CCGT"], "unitsUpperLimit"] = .0077  # GW_el # 0.167
converter_capacityParam.loc[idx["R1_data", :, "CCGT"], "unitsLowerLimit"] = 0.0075
converter_capacityParam.loc[idx["R1_data", :, "BG"], "unitsUpperLimit"] = 1  # GW_el # 0.167
converter_capacityParam.loc[idx["R1_data", :, "BG"], "unitsLowerLimit"] = 0   # GW_el # 0.0165
converter_capacityParam.loc[idx["R1_data", :, "PV"], "unitsUpperLimit"] = 0.007  # GW_el
...
converter_capacityParam.loc[idx["R1_data", :, "Wave"], "unitsUpperLimit"] = (
   1  # GW_el
)
converter_capacityParam.loc[idx["R1_data", :, "Wave"], "unitsLowerLimit"] = (
    0  # GW_el
)
converter_capacityParam.loc[idx["R1_data", :, "WindOffshore"], "unitsUpperLimit"] = (
  1  # GW_el
)
converter_capacityParam.loc[idx["R1_data", :, "WindOffshore"], "unitsLowerLimit"] = (
    0  # GW_el
)
...
converter_coefficient = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [
            ["CCGT", "BG", "PV", "WindOnshore", "Wave", "WindOffshore"],
            m.set.yearssel,
            ["Powergen"],
            ["CH4", "Biomass", "Elec", "CO2", "Heat"],
        ]
    )
)
converter_coefficient.loc[idx["CCGT", :, :, "Elec"], "coefficient"] = 1  # GWh_el
converter_coefficient.loc[idx["CCGT", :, :, "CH4"], "coefficient"] = -2.85  # GWh_ch
converter_coefficient.loc[idx["CCGT", :, :, "CO2"], "coefficient"] = 0.700  # kt CO2

converter_coefficient.loc[idx["BG", :, :, "Elec"], "coefficient"] = 1  # GWh_el
converter_coefficient.loc[idx["BG", :, :, "Biomass"], "coefficient"] = -4  # GWh_ch
converter_coefficient.loc[idx["BG", :, :, "CO2"], "coefficient"] = 0.2
converter_coefficient.loc[idx["BG", :, :, "Heat"], "coefficient"] = .5


converter_coefficient.loc[idx["PV", :, :, "Elec"], "coefficient"] = 1  # GWh_el

converter_coefficient.loc[idx["WindOnshore", :, :, "Elec"], "coefficient"] = 1

converter_coefficient.loc[idx["Wave", :, :, "Elec"], "coefficient"] = 1  
converter_coefficient.loc[idx["WindOffshore", :, :, "Elec"], "coefficient"] = 1  # GWh_el
...
converter_activityProfile = profiles[["PV", "WindOnshore", "Wave", "WindOffshore"]]
...
] = 160.0  # Mio EUR per unit

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "BG", "2030"], "perUnitBuild"
] = 400.0  # Mio EUR per unit
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "BG", "2030"], "useAnnuity"
] = 1  # binary yes/no
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "BG", "2030"], "amorTime"
] = 2  # years
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "BG", "2030"], "interest"
] = 0.06  # percent/100
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "BG", "2030"], "perUnitTotal"
] = 160.0
...
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "Wave", "2030"], "perUnitBuild"
] = 4000
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "Wave", "2030"], "useAnnuity"
] = 1
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "Wave", "2030"], "amorTime"
] = 25
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "Wave", "2030"], "interest"
] = 0.06
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "Wave", "2030"], "perUnitTotal"
] = 22

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOffshore", "2030"], "perUnitBuild"
] = 2500
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOffshore", "2030"], "useAnnuity"
] = 1
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOffshore", "2030"], "amorTime"
] = 25
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOffshore", "2030"], "interest"
] = 0.025
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "WindOffshore", "2030"], "perUnitTotal"
] = 22
...
# "sourcesink_profile"
# load the profiles DataFrame, select the demand column
sourcesink_profile = profiles[["Heating"]]
...
sourcesink_profile["years"] = "2030"
sourcesink_profile["techs"] = "Demand"
sourcesink_profile["commodity"] = "Heat"
sourcesink_profile["type"] = "fixed"
sourcesink_profile = sourcesink_profile.set_index(
    ["nodesData", "years", "techs", "commodity", "type"]
)
...
sourcesink_config = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [m.set.nodesdata, m.set.yearssel, ["Demand"], ["Heat"]]
    )
)
sourcesink_config.loc[idx["R1_data", :, :, :], "usesFixedProfile"] = 1
sourcesink_config = sourcesink_config.dropna()
...
accounting_sourcesinkFlow["perFlow"] = 0.0905  # Mio EUR per GWh_ch CH4
...
accounting_sourcesinkFlow["perFlow"] = 0.006  # Mio EUR per GWh_ch CH4
...
accounting_storageUnits.loc[idx["Invest", :, :, :, :], "interest"] = 0.07
