...
accounting_indicatorBounds_global.loc[idx[:, :, "SystemCost"], "endyear"] = (
    10  # duration of the last period (used for discounting cost)
)
accounting_indicatorBounds_global.loc[idx[:, :, "SystemCost"], "integral"] = (
    1  # if enabled, indicator will also be accounted for years between yearssel
)
accounting_indicatorBounds_global.loc[idx[:, :, "SystemCost"], "discount"] = (
...
accounting_indicatorBounds_global.loc[idx[:, :, "Carbon"], "upperValue"] = (
    8000000.0  # rounded value
)
accounting_indicatorBounds_global.loc[idx[:, :, "Carbon"], "endyear"] = (
    10  # duration of the last period (used for discounting emissions)
)
accounting_indicatorBounds_global.loc[idx[:, :, "Carbon"], "integral"] = (
    1  # if enabled, indicator will also be accounted for years between yearssel
)
accounting_indicatorBounds_global.loc[idx[:, :, "Carbon"], "useUpper"] = (
    1  # parameter to indicate the model to use the `upperValue` field
)

accounting_indicatorBounds_global.loc[idx[:, :, "Slack_Carbon"], "isVariable"] = (
    1  # parameter to indicate that this is a free variable
)
accounting_indicatorBounds_global.loc[idx[:, :, "Slack_Carbon"], "useLower"] = (
...
converter_techParam.loc[idx[["CCGT", "OCGT", "GreenImport"], :], "lifeTime"] = (
    30  # years
)
...
converter_coefficient.loc[idx["CCGT", :, "Powergen", "Elec"], "coefficient"] = (
    1  # GWh_el
)
...
converter_coefficient.loc[idx["OCGT", :, "Powergen", "Elec"], "coefficient"] = (
    1  # GWh_el
)
converter_coefficient.loc[
...
converter_coefficient.loc[idx["WindOnshore", :, "Powergen", "Elec"], "coefficient"] = (
    1  # GWh_el
)
converter_coefficient.loc[idx["GreenImport", :, "Import", "CH4"], "coefficient"] = (
    1  # GWh_ch
)
...
converter_coefficient.loc[idx["LiIon", :, "Charge", "Elec_LiIon"], "coefficient"] = (
    0.95  # GWh_el
)

converter_coefficient.loc[idx["LiIon", :, "Discharge", "Elec"], "coefficient"] = (
    0.95  # GWh_el
)
converter_coefficient.loc[
...
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "CCGT", "2020"], "perUnitBuild"
] = 700.0  # Mio EUR per unit
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "CCGT", "2020"], "useAnnuity"
] = 1  # binary yes/no
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "CCGT", "2020"], "amorTime"
] = 30  # years
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "CCGT", "2020"], "interest"
] = 0.06  # percent/100
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "CCGT", "2020"], "perUnitTotal"
] = 28.0  # Mio EUR per unit

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "OCGT", "2020"], "perUnitBuild"
] = 400.0  # Mio EUR per unit
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "OCGT", "2020"], "useAnnuity"
] = 1  # binary yes/no
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "OCGT", "2020"], "amorTime"
] = 30  # years
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "OCGT", "2020"], "interest"
] = 0.06  # percent/100
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "OCGT", "2020"], "perUnitTotal"
] = 16.0  # Mio EUR per unit

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "LiIon", "2020"], "perUnitBuild"
] = 50.0  # Mio EUR per unit
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "LiIon", "2020"], "useAnnuity"
] = 1  # binary yes/no
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "LiIon", "2020"], "amorTime"
] = 25  # years
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "LiIon", "2020"], "interest"
] = 0.06  # percent/100
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "LiIon", "2020"], "perUnitTotal"
] = 0.75  # Mio EUR per unit

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "PV", "2020"], "perUnitBuild"
] = 518.0
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "PV", "2020"], "useAnnuity"
] = 1
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "PV", "2020"], "amorTime"
] = 20
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "PV", "2020"], "interest"
] = 0.06
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "PV", "2020"], "perUnitTotal"
] = 7.7
accounting_converterUnits.loc[
    idx["Delta_PV", "global", "horizon", "PV", "2020"], "perUnitBuild"
] = 1
accounting_converterUnits.loc[
    idx["Delta_PV", "global", "horizon", "PV", "2020"], "perUnitDecom"
] = -1

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "GreenImport", "2040"], "perUnitBuild"
] = 500.0
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "GreenImport", "2040"], "useAnnuity"
] = 1
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "GreenImport", "2040"], "amorTime"
] = 30
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "GreenImport", "2040"], "interest"
] = 0.06
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "GreenImport", "2040"], "perUnitTotal"
] = 25

accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOnshore", "2020"], "perUnitBuild"
] = 1173.0
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOnshore", "2020"], "useAnnuity"
] = 1
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOnshore", "2020"], "amorTime"
] = 25
accounting_converterUnits.loc[
    idx["Invest", "global", "horizon", "WindOnshore", "2020"], "interest"
] = 0.06
accounting_converterUnits.loc[
    idx["OMFix", "global", "horizon", "WindOnshore", "2020"], "perUnitTotal"
] = 25.8
accounting_converterUnits.loc[
    idx["Delta_Wind", "global", "horizon", "WindOnshore", "2020"], "perUnitBuild"
] = 1
accounting_converterUnits.loc[
    idx["Delta_Wind", "global", "horizon", "WindOnshore", "2020"], "perUnitDecom"
] = -1
accounting_converterUnits = accounting_converterUnits.dropna(how="all").fillna(0)
...
accounting_sourcesinkFlow.loc[idx["FuelCost", :, "2020", :], "perFlow"] = (
    0.0306  # Mio EUR per GWh_ch CH4
)
accounting_sourcesinkFlow.loc[idx["FuelCost", :, "2030", :], "perFlow"] = (
    0.0350  # Mio EUR per GWh_ch CH4
)
accounting_sourcesinkFlow.loc[idx["FuelCost", :, "2040", :], "perFlow"] = (
    0.0400  # Mio EUR per GWh_ch CH4
)
accounting_sourcesinkFlow.loc[idx["FuelCost", :, "2050", :], "perFlow"] = (
...
] = -0.00004  # electrical losses of 40 kWh / h for each flow of 1 GWh / h and 1 km line length ~ 24 MWh / h for 600 km length
...
transfer_indicators = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["Invest", "OMFix"], area, horizon, transfer_techs, ["2020"]]
    )
)
...
indicators_length.loc[idx["OMVar", "global", "horizon", :, :, "land"], "perFlow"] = (
    0.00002
)
...
indicators_length.loc[idx["OMVar", "global", "horizon", :, :, "sea"], "perFlow"] = (
    0.00002
)
...
accounting_storageUnits = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["Invest", "OMFix"], ["global"], ["horizon"], ["LiIon"], ["2020"]]
    )
)
