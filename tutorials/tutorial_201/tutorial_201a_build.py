...
ev_parameters.loc[idx["R3_data", :, "EVs_UC"], "EVs_Number"] = (
    ev_uncontrolled_vehicle_number["R3_data"]
)
ev_parameters.loc[idx["R1_data", :, "EVs_UC"], "EVs_Number"] = (
    ev_uncontrolled_vehicle_number["R1_data"]
)
ev_parameters.loc[idx["R2_data", :, "EVs_UC"], "EVs_Number"] = (
    ev_uncontrolled_vehicle_number["R2_data"]
)
ev_parameters.loc[idx["R4_data", :, "EVs_UC"], "EVs_Number"] = (
    ev_uncontrolled_vehicle_number["R4_data"]
)

# number of controlled charged vehicles
ev_parameters.loc[idx["R3_data", :, "EVs_CC"], "EVs_Number"] = (
    ev_controlled_vehicle_number["R3_data"]
)
ev_parameters.loc[idx["R1_data", :, "EVs_CC"], "EVs_Number"] = (
    ev_controlled_vehicle_number["R1_data"]
)
ev_parameters.loc[idx["R2_data", :, "EVs_CC"], "EVs_Number"] = (
    ev_controlled_vehicle_number["R2_data"]
)
ev_parameters.loc[idx["R4_data", :, "EVs_CC"], "EVs_Number"] = (
...
accounting_converterUnits = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["Invest", "OMFix"], ["global"], ["horizon"], ["EVs_CC"], m.set.yearssel]
    )
).sort_index()
accounting_converterUnits.loc[idx["Invest", :, :, "EVs_CC", :], "perUnitBuild"] = (
    0.0  # Mio EUR / unit
)
...
charging_avail = charging_avail.rename(
    columns={"Unnamed: 0": "tech", "Unnamed: 1": "timestamp"}
)
...
charging_avail = charging_avail.rename(
    columns={"value": "chargingAvailabililty", "variable": "region"}
)
...
max_capacity = charging_avail_scaled_stack.groupby(["region", "year", "tech"]).agg(
    "max"
)
...
storage_techParam.loc[idx["EV_Storage", :], "levelUpperLimit"] = (
    1  # does not come into play
)
storage_techParam.loc[idx["EV_Storage", :], "levelLowerLimit"] = (
...
accounting_storageUnits = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["Invest", "OMFix"], ["global"], ["horizon"], ["EV_Storage"], m.set.yearssel]
    )
)
...
SoCMin = SoCMin.rename(columns={"Unnamed: 0": "tech", "Unnamed: 1": "timestamp"})
SoCMin["type"] = "lower"
SoCMin["years"] = "2030"
...
SoCMin = SoCMin.rename(columns={"value": "SoCMin", "variable": "region"})
...
SoCMax = SoCMax.rename(columns={"Unnamed: 0": "tech", "Unnamed: 1": "timestamp"})
SoCMax["type"] = "upper"
SoCMax["years"] = "2030"
...
SoCMax = SoCMax.rename(columns={"value": "SoCMax", "variable": "region"})
...
demand_driving = demand_driving.rename(
    columns={"Unnamed: 0": "tech", "Unnamed: 1": "timestamp"}
)
...
demand_driving = demand_driving.rename(
    columns={"value": "driving_energy", "variable": "region"}
)
...
converter_coefficient.loc[idx["EVs_UC", :, "Charge", "EV_Stored"], "coefficient"] = (
...
accounting_converterUnits = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["Invest", "OMFix"], ["global"], ["horizon"], ["EVs_UC"], m.set.yearssel]
    )
).sort_index()
accounting_converterUnits.loc[idx["Invest", :, :, "EVs_UC", :], "perUnitBuild"] = (
    0.0  # Mio EUR / unit
)
...
accounting_converterActivity = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [["OMVar"], ["global"], ["horizon"], ["EVs_UC"], m.set.yearssel, ["Charge", "Discharge"]]
    )
)
accounting_converterActivity["perActivity"] = 0.0
...
charging_avail_uc = charging_avail_uc.rename(
    columns={"Unnamed: 0": "tech", "Unnamed: 1": "timestamp"}
)
